...
export default function AlbumSection({ containerRef, onDarkChange }) {
  ...
  useEffect(() => { if (l3) draw(c3, i3, 0, '#000', -40) }, [l3, draw, i3])
  ...
  */
  ...
  useMotionValueEvent(idx3, 'change', (v) => { if (l3) draw(c3, i3, Math.max(0, Math.round(v)), '#000', -40) })

  // Track when frames_2 is active (dark background) and notify parent
  const [isDark, setIsDark] = useState(false)
  useMotionValueEvent(scrollYProgress, 'change', (v) => {
    const dark = v >= 0.20 && v < 1.0
    setIsDark(dark)
